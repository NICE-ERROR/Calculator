const periodicTable = [
    ...
    { symbol: "I", name: "Iodine", atomicNumber: 53, atomicMass: 126.9 },
    ...
    { symbol: "Dy", name: "Dysprosium", atomicNumber: 66, atomicMass: 162.5 },
...
function searchElement() {
    const symbolInput = document.getElementById("elementInput").value.trim();
    console.log("Input:", symbolInput);  // ตรวจสอบค่าที่ได้จาก input

    const element = periodicTable.find(el => el.symbol.toLowerCase() === symbolInput.toLowerCase());
    console.log("Found element:", element);  // ตรวจสอบว่าพบธาตุหรือไม่

    if (element) {
        document.getElementById("elementResult").innerText =
            `ชื่อธาตุ: ${element.name}, เลขอะตอม: ${element.atomicNumber}, มวลอะตอม: ${element.atomicMass}`;
    } else {
        document.getElementById("elementResult").innerText = "ไม่พบธาตุที่ค้นหา";
    }
}
